...
function Transaction () {
  this.version = 1  // Pre overwinter
  this.overwintered = 0  // 0x80 for overwinter
...
Transaction.fromBuffer = function (buffer, __noStrict, zcash) {
  var offset = 0
  ...
  if (zcash && tx.overwintered && tx.version >= 3) {
  ...
  if (tx.fOverwintered || (tx.version >= 2 && zcash)) {
...
Transaction.fromHex = function (hex, zcash) {
  return Transaction.fromBuffer(Buffer.from(hex, 'hex'), zcash)
}
