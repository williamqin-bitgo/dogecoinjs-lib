...
import * as fixtures from './fixtures/transaction_builder_bigint.json';

function constructSign(
  f: any,
  txb: any,
  useOldSignArgs: any,
): TransactionBuilder<bigint> {
  ...
  f.inputs.forEach((input: any, index: number) => {
    ...
    input.signs.forEach((sign: any) => {
      ...
      if (sign.stage) {
        ...
        txb = TransactionBuilder.fromTransaction<bigint>(tx, network);
      }
    });
  });
...
function construct(
  ...
  const network = (NETWORKS as any)[f.network];
  const txb = new TransactionBuilder<bigint>(network);
  ...
  f.inputs.forEach((input: any) => {
    ...
    if (input.txRaw) {
      ...
      prevTx = Transaction.fromHex<bigint>(input.txHex, 'bigint');
    } else {
      prevTx = input.txId;
    }
    ...
    txb.addInput(prevTx, input.vout, input.sequence, prevTxScript, inputValue);
  });
...
for (const useOldSignArgs of [false, true]) {
  ...
  describe(`TransactionBuilder: useOldSignArgs === ${useOldSignArgs}`, () => {
    ...
    describe('fromTransaction', () => {
      fixtures.valid.build.forEach((f: any) => {
        it('returns TransactionBuilder, with ' + f.description, () => {
          ...
          const tx = Transaction.fromHex<bigint>(f.txHex, 'bigint');
          const txb = TransactionBuilder.fromTransaction<bigint>(tx, network);
      ...
      fixtures.valid.fromTransaction.forEach((f: any) => {
        it('returns TransactionBuilder, with ' + f.description, () => {
          const tx = new Transaction<bigint>();
          ...
          const txb = TransactionBuilder.fromTransaction<bigint>(tx);
          const txAfter = f.incomplete ? txb.buildIncomplete() : txb.build();
      ...
      it('classifies transaction inputs', () => {
        const tx = Transaction.fromHex<bigint>(fixtures.valid.classification.hex, 'bigint');
        const txb = TransactionBuilder.fromTransaction<bigint>(tx);
      ...
      fixtures.invalid.fromTransaction.forEach((f: any) => {
        it('throws ' + f.exception, () => {
          ...
          assert.throws(() => {
            TransactionBuilder.fromTransaction<bigint>(tx);
    ...
    describe('addInput', () => {
      let txb: TransactionBuilder<bigint>;
      beforeEach(() => {
        txb = new TransactionBuilder<bigint>();
      });
      ...
      it('accepts a txHash, index [, sequence number and scriptPubKey]', () => {
        ...
        assert.strictEqual(txIn.sequence, 54);
        assert.strictEqual(
          (txb as any).__INPUTS[0].prevOutScript,
          scripts[1],
        );
      });

      it('accepts a prevTx, index [and sequence number]', () => {
        const prevTx = new Transaction<bigint>();
    ...
    describe('setLockTime', () => {
      it('throws if if there exist any scriptSigs', () => {
        const txb = new TransactionBuilder<bigint>();
        txb.addInput(txHash, 0);
    ...
    describe('sign', () => {
      ...
      it('supports low R signature signing', () => {
        let txb = new TransactionBuilder<bigint>();
        txb.setVersion(1);
        txb.addInput(
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff',
          1,
            ...
            (f.inputs as any).forEach(
              (input: any, index: number): void => {
                input.signs.forEach((sign: any) => {
                  ...
                  let witnessValue: bigint | undefined;
    ...
    describe('build', () => {
        ...
        describe('for ' + (f.description || f.exception), () => {
          it('throws ' + f.exception, () => {
            assert.throws(() => {
              let txb;
              if (f.txHex) {
                txb = TransactionBuilder.fromTransaction<bigint>(
                  Transaction.fromHex<bigint>(f.txHex, 'bigint'),
                );
              } else {
      ...
      it('for incomplete with 0 signatures', () => {
        ...
        const randomTx = Transaction.fromHex<bigint>(randomTxData, 'bigint');
        const txb = new TransactionBuilder<bigint>();
      ...
      it('for incomplete P2SH with 0 signatures', () => {
        ...
        const inpTx = Transaction.fromBuffer<bigint>(inp, undefined, 'bigint');

        const txb = new TransactionBuilder<bigint>(NETWORKS.testnet);
      ...
      it('for incomplete P2WSH with 0 signatures', () => {
        const inpTx = Transaction.fromBuffer<bigint>(
          Buffer.from(
    ...
    describe('various edge case', () => {
      ...
      it('should warn of high fee for segwit transaction based on VSize, not Size', () => {
        ...
        const txb = TransactionBuilder.fromTransaction<bigint>(
          Transaction.fromHex<bigint>(rawtx, 'bigint'),
        );
        (txb as any).__INPUTS[0].value = BigInt(241530);
        (txb as any).__INPUTS[1].value = BigInt(241530);
        (txb as any).__INPUTS[2].value = BigInt(248920);
        (txb as any).__INPUTS[3].value = BigInt(248920);
      ...
      it('should classify witness inputs with witness = true during multisigning', () => {
        ...
        TransactionBuilder.fromTransaction<bigint>(Transaction.fromHex<bigint>(txHex, 'bigint'));
      });
      ...
      it('should handle badly pre-filled OP_0s', () => {
        ...
        const txb = TransactionBuilder.fromTransaction<bigint>(tx, NETWORKS.testnet);
      ...
      it('should not classify blank scripts as nonstandard', () => {
        ...
        // and, repeat
        txb = TransactionBuilder.fromTransaction<bigint>(
          Transaction.fromHex<bigint>(incomplete, 'bigint'),
        );
        txb.addOutput('1Gokm82v6DmtwKEB8AiVhm82hyFSsEvBDK', BigInt(15000));
        txb.sign({
          prevOutScriptType: 'p2pkh',
