...
const fixtureNames = ['transaction_builder', 'transaction_builder_bignum'];
...
for (const fxsName of fixtureNames) {
  const rawFixture = fs.readFileSync(
    path.resolve(__dirname, `./fixtures/${fxsName}.json`),
  );
  ...
  for (const useOldSignArgs of [false, true]) {
    ...
    describe(`TransactionBuilder: useOldSignArgs === ${useOldSignArgs}, fixture === ${fxsName}`, () => {
      // constants
      ...
      describe('addInput', () => {
        ...
        it('accepts a prevTx, index [and sequence number]', () => {
          ...
          prevTx.addOutput(scripts[0], new BigNumber(0));
          prevTx.addOutput(scripts[1], new BigNumber(1));
        ...
        it('throws if SIGHASH_ALL has been used to sign any existing scriptSigs', () => {
          ...
          txb.addOutput(scripts[0], new BigNumber(1000));
          txb.sign({
      ...
      describe('addOutput', () => {
        ...
        it('accepts an address string and value', () => {
          ...
          const vout = txb.addOutput(address!, new BigNumber(1000));
          assert.strictEqual(vout, 0);

          const txout = (txb as any).__TX.outs[0];
          assert.deepStrictEqual(txout.script, scripts[0]);
          assert.ok(txout.value.eq(1000));
        });

        it('accepts a ScriptPubKey and value', () => {
          const vout = txb.addOutput(scripts[0], new BigNumber(1000));
          assert.strictEqual(vout, 0);
        ...
        it('add second output after signed first input with SIGHASH_NONE', () => {
          txb.addInput(txHash, 0);
          txb.addOutput(scripts[0], new BigNumber(2000));
          txb.sign({
            prevOutScriptType: 'p2pkh',
            vin: 0,
            keyPair,
          ...
          assert.strictEqual(txb.addOutput(scripts[1], new BigNumber(9000)), 1);
        });
        ...
        it('add first output after signed first input with SIGHASH_NONE', () => {
          ...
          assert.strictEqual(txb.addOutput(scripts[0], new BigNumber(2000)), 0);
        });
        ...
        it('throws if SIGHASH_ALL has been used to sign any existing scriptSigs', () => {
          ...
          assert.throws(() => {
            txb.addOutput(scripts[1], new BigNumber(9000));
          }, /No, this would invalidate signatures/);
        });
      });
      ...
      describe('setLockTime', () => {
        it('throws if if there exist any scriptSigs', () => {
          ...
          txb.addInput(txHash, 0);
          txb.addOutput(scripts[0], new BigNumber(100));
          txb.sign({
            prevOutScriptType: 'p2pkh',
            vin: 0,
            keyPair,
      ...
      describe('sign', () => {
        it('supports the alternative abstract interface { publicKey, sign }', () => {
          ...
          txb.addOutput('1111111111111111111114oLvT2', new BigNumber(100000));
          txb.sign({
            prevOutScriptType: 'p2pkh',
            vin: 0,
              ...
              (f.inputs as any).forEach(
                (input: any, index: number): void => {
                  input.signs.forEach((sign: any) => {
                    ...
                    let witnessValue: BigNumber | undefined;
      ...
      describe('build', () => {
        ...
        it('for incomplete with 0 signatures', () => {
          ...
          txb.addOutput(randomAddress, new BigNumber(1000));
          const tx = txb.buildIncomplete();
          assert(tx);
        });
        ...
        it('for incomplete P2SH with 0 signatures', () => {
          ...
          txb.addOutput(
            '2NAkqp5xffoomp5RLBcakuGpZ12GU4twdz4',
            new BigNumber(1e8),
          ); // arbitrary output
      ...
      describe('various edge case', () => {
        ...
        it('should classify witness inputs with witness = true during multisigning', () => {
          ...
          txb.addOutput(scriptPubKey, new BigNumber(99000));
          txb.sign({
            prevOutScriptType: 'p2sh-p2wsh-p2ms',
            vin: 0,
            keyPair: innerKeyPair,
            redeemScript,
            witnessValue: new BigNumber(100000),
            witnessScript,
          });
